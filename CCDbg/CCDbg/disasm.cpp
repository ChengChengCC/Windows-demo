...
static void DecodeRJ(ulong offsize,ulong nextip, ulong BaseAddr) {
  int i;
  ulong addr;
  char s[TEXTLEN];
  if (size<offsize+1) {
  ...
  if (datasize==2)
    addr&=0xFFFF;
  da->jmpconst=addr;
  // modify by sun. 2010.3.23
  // change the 'addr' from relative address to absolute address
  addr += BaseAddr;
  if (addr==0) da->zeroconst=1;
  if (mode>=DISASM_FILE) {
...
ulong Disasm(char *src,ulong srcsize,ulong srcip,
  t_disasm *disasm,int disasmmode, ulong BaseAddr) {
  ...
  if (pd->mask==0) {                   // Command not found
    ...
    for (operand=0; operand<3; operand++) {
      ...
      switch (arg) {
        ...
        // modify by sun. 2010.3.23
        // change the 'addr' from relative address to absolute address
          DecodeRJ(1,srcip+2, BaseAddr); break;
        case JOW:                      // Immediate full offset (for jumps)
          DecodeRJ(datasize,srcip+datasize+1, BaseAddr); break;
